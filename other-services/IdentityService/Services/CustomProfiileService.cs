...
public class CustomProfiileService(UserManager<ApplicationUser> userManager) : IProfileService
{
    ...
    {
        var user =
            await userManager.GetUserAsync(context.Subject) ?? throw new InvalidOperationException("User not found");
        var existingClaims = await userManager.GetClaimsAsync(user);

        var claims = new List<Claim> { new("username", user.UserName ?? string.Empty) };
